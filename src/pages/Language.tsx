    ...
    <Layout>
      ...
      {query.trim() ? (
        <ul className="sheet-list">
          ...
          {results.map((r) => (
            <li key={r.slug}>
              <Link to={`/lang/${encodeURIComponent(group.language)}/${encodeURIComponent(r.slug)}`} className="sheet-link">{r.title}</Link>
        ...
        <ul className="sheet-list">
          <li>
            <Link to={`/lang/${encodeURIComponent(group.language)}/combined`} className="sheet-link combined">
              Combined (all {group.sheets.length})
            </Link>
          </li>
          {group.sheets.map((s) => (
            <li key={s.slug}>
              <Link to={`/lang/${encodeURIComponent(group.language)}/${encodeURIComponent(s.slug)}`} className="sheet-link">{s.title}</Link>
